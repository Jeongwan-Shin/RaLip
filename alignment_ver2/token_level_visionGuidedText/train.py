...
def train_step(model, pts, mask, texts, neg_mask, images=None, text_type=None):
    ...
    it = model._get_image_tokens(images, pts.device)
    tc = model.encode_text_cls(texts, pts.device)
...
if __name__ == "__main__":
    ...
    args = p.parse_args()
    if not args.run_tag: args.run_tag = f"v2_tl_vgt_ep{args.epochs}_bs{args.batch_size}"
