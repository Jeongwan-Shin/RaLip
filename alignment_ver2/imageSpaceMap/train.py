...
def train_step(model, pts, mask, texts, neg_mask, images=None, text_type=None):
    ...
    loss, acc = masked_contrastive_loss(pe, ie, ls)  # no text masking for image alignment
    return {"loss": loss, "acc": acc.detach()}

if __name__ == "__main__":
    p = add_common_args(argparse.ArgumentParser())
    args = p.parse_args()
    if not args.run_tag: args.run_tag = f"v2_imgspace_ep{args.epochs}_bs{args.batch_size}"
    ...
    cfg = RadarCLIPVisionConfig(embed_dim=args.embed_dim, out_dim=args.out_dim, num_layers=args.num_layers,
                                num_heads=args.num_heads, proj_dim=args.proj_dim, pooling="cls")
    model = RadarCLIPVision(cfg)
