...
def train_step(model, pts, mask, texts, neg_mask, images=None, text_type=None):
    ...
    vt, vc = model._get_vision_tokens_and_cls(pts, mask)
    cross = model.cross_attn(pt, vt, mask)
    stand = model.standalone_pool(pt, mask)
    lc, _ = masked_contrastive_loss(cross, vc, ls)
    ls2, acc = masked_contrastive_loss(stand, vc, ls)
...
if __name__ == "__main__":
    ...
    args = p.parse_args()
    if not args.run_tag: args.run_tag = f"v2_tl_ptvis_ep{args.epochs}_bs{args.batch_size}"
    ...
    cfg = TokenLevelPtVisionConfig(embed_dim=args.embed_dim, num_layers=args.num_layers,
                                   num_heads=args.num_heads, proj_dim=args.proj_dim,
                                   cross_heads=args.cross_heads, aux_weight=args.aux_weight,
                                   max_points=args.max_points)
    model = TokenLevelPtVision(cfg)
    ...
    run_training(args, model, params, model.encode_points_standalone, model.encode_text, train_step)
