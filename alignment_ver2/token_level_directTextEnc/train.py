...
def train_step(model, pts, mask, texts, neg_mask, images=None, text_type=None):
    ...
    tt, tc = model._get_text_tokens_and_cls(texts, pts.device)
    cross = model.cross_attn(pt, tt, mask)
    stand = model.standalone_pool(pt, mask)
    lc, _ = masked_contrastive_loss(cross, tc, ls, neg_mask)
    ls2, acc = masked_contrastive_loss(stand, tc, ls, neg_mask)
...
if __name__ == "__main__":
    ...
    args = p.parse_args()
    if not args.run_tag: args.run_tag = f"v2_tl_direct_ep{args.epochs}_bs{args.batch_size}"
    ...
    cfg = TokenLevelDirectTextConfig(embed_dim=args.embed_dim, num_layers=args.num_layers,
                                     num_heads=args.num_heads, proj_dim=args.proj_dim,
                                     cross_heads=args.cross_heads, aux_weight=args.aux_weight)
    model = TokenLevelDirectText(cfg)
    ...
    run_training(args, model, params, model.encode_points_standalone, model.encode_text_cls, train_step)
