"""
Token-level visionGuidedText: cross-attention between point cloud tokens
and CLIP vision tokens, contrastive with text.

Training:
  cross_attn(point_tokens, image_tokens) → fused ↔ text_cls  (main loss)
  standalone_pool(point_tokens) → point_emb ↔ text_cls       (aux loss)

Testing:
  standalone_pool(point_tokens) → point_emb vs text_cls → R@K
"""
...
class TokenLevelVGT(nn.Module):
    def __init__(self, cfg: TokenLevelVGTConfig):
        super().__init__()
        self.cfg = cfg
        ...
        # Cross-attention: point(256) × image(768)
    ...
    def _get_image_tokens(self, images, device):
        inputs = self.processor(images=images, return_tensors="pt").to(device)
        with torch.no_grad():
            return self.clip_model.vision_model(**inputs).last_hidden_state  # (B,50,768)
    ...
    def encode_text_cls(self, texts, device):
        ...
        with torch.no_grad(): return F.normalize(self.clip_model.get_text_features(**tok), dim=-1)

    def forward(self, points, mask, images, texts):
        logit_scale = self.logit_scale.exp().clamp(max=100.0)
        point_tokens = self._get_point_tokens(points, mask)
        img_tokens = self._get_image_tokens(images, points.device)
        text_cls = self.encode_text_cls(texts, points.device)

        # Main: cross_attn(point, image) ↔ text
