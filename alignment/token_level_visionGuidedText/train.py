"""Token-level visionGuidedText training. Train: point×image→text. Test: point→text."""
...
from model import TokenLevelVGT, TokenLevelVGTConfig, load_pretrained_token_encoder

# Dataset from visionGuidedText
_vgt_dir = os.path.abspath(os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "visionGuidedText"))
sys.path.insert(0, _vgt_dir)
import dataset as _ds
sys.path.pop(0)
RadarImageTextDataset = _ds.RadarImageTextDataset
build_entries = _ds.build_entries
split_entries = _ds.split_entries
collate_radar_image_text = _ds.collate_radar_image_text
...
def evaluate(model, test_entries, dataset, device, max_points=196, batch_size=64, desc="eval"):
    ...
    for i in tqdm_maybe(range(len(dataset)), desc=f"{desc} enc-radar", unit="s"):
        e = test_entries[i]; pts_t, _, _ = dataset[i]
...
def parse_args():
    ...
    p.add_argument("--ckpt_dir", default="alignment/token_level_visionGuidedText/checkpoints")
    p.add_argument("--log_dir", default="alignment/token_level_visionGuidedText/log")
...
def main():
    args = parse_args()
    tag = args.run_tag or f"tl_vgt_ep{args.epochs}_bs{args.batch_size}_lr{args.lr}_aux{args.aux_weight}"
    ...
    train_ds = RadarImageTextDataset(train_e, **dk, train=True)
    test_ds = RadarImageTextDataset(test_e, **dk, train=False)
    ...
    cfg = TokenLevelVGTConfig(embed_dim=args.embed_dim, num_layers=args.num_layers, num_heads=args.num_heads,
                              proj_dim=args.proj_dim, cross_heads=args.cross_heads, aux_weight=args.aux_weight)
    model = TokenLevelVGT(cfg).to(device)
    ...
    for epoch in range(args.epochs):
        ...
        for step, (pts, mask, images, texts) in pbar:
            ...
            out = model(pts, mask, images, texts); loss = out["loss"]
