"""token_level_ptVision training. Train: point×vision(tokenized). Test: point→text."""
...
from model import TokenLevelPtVision, TokenLevelPtVisionConfig, load_pretrained_token_encoder
...
def parse_args():
    ...
    p.add_argument("--ckpt_dir", default="alignment/token_level_ptVision/checkpoints")
    p.add_argument("--log_dir", default="alignment/token_level_ptVision/log")
...
def main():
    args = parse_args()
    tag = args.run_tag or f"tl_ptvis_ep{args.epochs}_bs{args.batch_size}_lr{args.lr}_aux{args.aux_weight}"
    ...
    cfg = TokenLevelPtVisionConfig(embed_dim=args.embed_dim, num_layers=args.num_layers, num_heads=args.num_heads,
                                   proj_dim=args.proj_dim, cross_heads=args.cross_heads, aux_weight=args.aux_weight,
                                   max_points=args.max_points)
    model = TokenLevelPtVision(cfg).to(device)
    ...
    params = [{"params": model.token_encoder.parameters()}, {"params": model.pt_tokenizer.parameters()},
              {"params": model.cross_attn.parameters()}, {"params": model.standalone_pool.parameters()},
