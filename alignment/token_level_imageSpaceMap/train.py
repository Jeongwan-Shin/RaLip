"""Token-level imageSpaceMap training. Train: point×image. Test: point→text."""
...
# Dataset from imageSpaceMap
_img_dir = os.path.abspath(os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "imageSpaceMap"))
sys.path.insert(0, _img_dir)
import dataset as _ds
sys.path.pop(0)
RadarImageDataset = _ds.RadarImageDataset
build_entries = _ds.build_entries
split_entries = _ds.split_entries
collate_radar_image = _ds.collate_radar_image


def tqdm_maybe(it, **kw):
    try: from tqdm import tqdm; return tqdm(it, file=sys.__stdout__, dynamic_ncols=True, leave=True, **kw)
    except: return it
...
@torch.no_grad()
def evaluate(model, test_entries, test_text_entries, dataset, device, max_points=196, batch_size=64, desc="eval"):
    """R2T retrieval: standalone point → text."""
    model.eval()
    all_pe, all_qa = [], []
    for i in tqdm_maybe(range(len(dataset)), desc=f"{desc} enc-radar", unit="s"):
        ...
        all_pe.append(model.encode_points_standalone(pts.to(device), mask.to(device)).cpu())
        all_qa.append(e.action)
    point_embeds = torch.cat(all_pe, 0)

    all_t, all_ta = [], []
    for te in test_text_entries:
        for s in te["sentences"]: all_t.append(s); all_ta.append(te["annotation"]["action"])
    all_te = []
    for i in tqdm_maybe(range(0, len(all_t), batch_size), desc=f"{desc} enc-text", unit="b"):
        all_te.append(model.encode_text_cls(all_t[i:i+batch_size], device).cpu())
...
def parse_args():
    ...
    p.add_argument("--mmbody_pc_dir", required=True); p.add_argument("--mmfi_pc_dir", required=True); p.add_argument("--mri_pc_dir", required=True)
    p.add_argument("--mmbody_img_dir", required=True); p.add_argument("--mmfi_img_dir", required=True); p.add_argument("--mri_img_dir", required=True)
    ...
    p.add_argument("--scheduler", default="cosine"); p.add_argument("--warmup_epochs", type=int, default=5)
    p.add_argument("--min_lr", type=float, default=1e-6)
    p.add_argument("--ckpt_dir", default="alignment/token_level_imageSpaceMap/checkpoints")
    p.add_argument("--log_dir", default="alignment/token_level_imageSpaceMap/log")
...
def main():
    args = parse_args()
    tag = args.run_tag or f"tl_imgspace_ep{args.epochs}_bs{args.batch_size}_lr{args.lr}_aux{args.aux_weight}"
    ...
    test_actions = {e.action for e in test_e}
    test_text_entries = []
    _seen = set()
    with open(args.actions_text_jsonl, "r") as f:
        for line in f:
            obj = _json.loads(line.strip())
            act = obj["annotation"]["action"]
            if act in test_actions and act not in _seen:
                _seen.add(act); test_text_entries.append(obj)
    print(f"[info] test text entries: {len(test_text_entries)}")

    dk = dict(mmbody_pc_dir=args.mmbody_pc_dir, mmfi_pc_dir=args.mmfi_pc_dir, mri_pc_dir=args.mri_pc_dir,
              mmbody_img_dir=args.mmbody_img_dir, mmfi_img_dir=args.mmfi_img_dir, mri_img_dir=args.mri_img_dir,
              feature_norm=args.feature_norm)
    train_ds = RadarImageDataset(train_e, **dk, train=True)
    test_ds = RadarImageDataset(test_e, **dk, train=False)
    ...
    cfg = TokenLevelImageSpaceConfig(embed_dim=args.embed_dim, num_layers=args.num_layers, num_heads=args.num_heads,
                                     proj_dim=args.proj_dim, cross_heads=args.cross_heads, aux_weight=args.aux_weight)
    model = TokenLevelImageSpace(cfg).to(device)
    ...
    if args.scheduler == "cosine":
        ...
        scheduler = LambdaLR(optim, lambda s: (float(max(0,min(s,total)))/float(wu) if wu>0 and max(0,min(s,total))<wu else ((mn/base)+(1-mn/base)*0.5*(1+math.cos(math.pi*((max(0,min(s,total))-wu)/float(max(1,total-wu)))))) if total>wu else 1.0))

    print(f"[info] trainable={sum(p.numel() for p in model.parameters() if p.requires_grad):,}")
    best_r10 = 0.0
    for epoch in range(args.epochs):
        ...
        for step, (pts, mask, images) in pbar:
            ...
            out = model(pts, mask, images); loss = out["loss"]
            optim.zero_grad(set_to_none=True); loss.backward(); optim.step()
            if scheduler: scheduler.step()
            if hasattr(pbar,"set_postfix"):
                pbar.set_postfix(loss=f"{float(loss):.4f}", cross=f"{float(out['acc_cross']):.2f}", stand=f"{float(out['acc_stand']):.2f}")

        metrics = evaluate(model, test_e, test_text_entries, test_ds, device, args.max_points, args.eval_batch_size, f"eval {epoch+1}/{args.epochs}")
