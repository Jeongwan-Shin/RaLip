"""ptVision (CLS) training. Train: point↔vision(tokenized). Test: point→text."""
...
# Dataset from directTextEnc (only need point cloud + text for eval)
...
import dataset as _ds; sys.path.pop(0)
RadarTextDataset, build_entries, split_entries, collate_radar_text = (
    _ds.RadarTextDataset, _ds.build_entries, _ds.split_entries, _ds.collate_radar_text)
...
def evaluate(model, test_entries, dataset, device, max_points=196, batch_size=64, desc="eval"):
    ...
    for i in tqdm_maybe(range(len(dataset)), desc=f"{desc} enc-radar", unit="s"):
        ...
        all_pe.append(model.encode_points(pts.to(device), mask.to(device)).cpu())
    ...
    for i in tqdm_maybe(range(0, len(all_t), batch_size), desc=f"{desc} enc-text", unit="b"):
        all_te.append(model.encode_text(all_t[i:i+batch_size], device).cpu())
...
def parse_args():
    ...
    p.add_argument("--embed_dim", type=int, default=256); p.add_argument("--out_dim", type=int, default=128)
    p.add_argument("--num_layers", type=int, default=4); p.add_argument("--num_heads", type=int, default=4)
    ...
    p.add_argument("--ckpt_dir", default="alignment/ptVision/checkpoints")
    p.add_argument("--log_dir", default="alignment/ptVision/log")
...
def main():
    args = parse_args()
    tag = args.run_tag or f"ptvis_ep{args.epochs}_bs{args.batch_size}_lr{args.lr}"
    ...
    model = PtVision(cfg).to(device)
    if args.pretrained_ckpt and os.path.isfile(args.pretrained_ckpt):
        load_pretrained_encoder(model.point_encoder, args.pretrained_ckpt, args.device)

    # Train: point encoder + point proj + tokenizer + logit_scale
    ...
    print(f"[info] trainable={nt:,}  device={device}  epochs={args.epochs}")
    ...
    for epoch in range(args.epochs):
        ...
        for step, (pts, mask, _texts) in pbar:
            if pts.shape[0]==0: continue
            pts, mask = pts.to(device), mask.to(device)
            out = model(pts, mask); loss = out["loss"]
            optim.zero_grad(set_to_none=True); loss.backward(); optim.step()
            if scheduler: scheduler.step()
            if hasattr(pbar,"set_postfix"):
                pbar.set_postfix(loss=f"{float(loss):.4f}", acc=f"{float(out['acc']):.2f}", lr=f"{optim.param_groups[0]['lr']:.2e}")
